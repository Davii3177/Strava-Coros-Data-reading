...
The included `render.yaml` provisions the web service. Configure `APP_PASSWORD`, `REGISTRATION_CODE`, and API credentials in Render. On first launch, visit `/register` and use `APP_PASSWORD` as the one-time setup code to make the owner account; then rotate or remove it and set `REGISTRATION_CODE` only when inviting another user.

Render's free filesystem is ephemeral. Account, race, feedback, shoe, plan, recovery, and connection-status files can reset after a service replacement until a durable database migration is enabled. The free service can also take roughly 30-60 seconds to wake after inactivity.
